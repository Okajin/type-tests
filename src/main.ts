...
export function runMain() {
  ...
  if (oldest) {
    console.log(`La personne la plus âgée est ${oldest.name}, ${oldest.age} ans.`);
  } else {
    console.log('Liste vide !');
  }

  const totalBuyout = calculateTotal(inventory);
  const totalExpensiveBuyout = roundToTwoDecimals(calculateExpensiveTotal(inventory));
  const totalDiscountedBuyout = roundToTwoDecimals(calculateDiscountedTotal(inventory));
